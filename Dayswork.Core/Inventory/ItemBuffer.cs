...
public sealed class ItemBuffer
{
    ...
    public void Add(string itemId, int quantity, TaskKind sourceTask, OutputScopeProvenance? provenance = null, int quality = 0, string? flavorId = null)
    {
        if (string.IsNullOrEmpty(itemId)) throw new ArgumentException("itemId must be non-empty.", nameof(itemId));
        if (quantity <= 0) throw new ArgumentOutOfRangeException(nameof(quantity), "quantity must be positive.");
        _items.Add(new BufferedItem(itemId, quantity, sourceTask, provenance ?? OutputScopeProvenance.Unknown, quality, flavorId));
    }
