...
- [~] **Step 3 — Grandpa's Shed work location (S-25).** PENDING — deferred pending runtime evidence of the shed-greenhouse location graph (entry/warp/selection); no dead data added.
- [~] **Step 4 (diagnostic added; impl pending) — Unique-name building keying (TODO-08). [highest risk — do last, playtest]**
  Added `[Dayswork][building-outline]` diagnostic (`ChestResolver.GetBuildingOutlines`) logging type/name/uniqueName/tile to confirm whether duplicate interiors have distinct `NameOrUniqueName` before changing the key (else use a coord discriminator). Implementation follows the evidence.
