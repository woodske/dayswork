...
- Review fixes:
  ...
  - worker action state snapshots restore the farmer's pre-action animation/tool flags and tool callbacks use a hidden proxy farmer instead of the real player
