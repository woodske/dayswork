...
U-15 adds one Mod component (`CalendarHandlers`), promotes the scheduler to the full daily lifecycle, adds an at-save sleep-stop settlement method to the orchestrator, changes `ToolLevelReader`'s missing-tool semantics, and extends `MailDispatcher` for mailed refunds — while removing the tool-missing warning path. The worker behavioural loop and the deposit planner are reused unchanged for normal shift completion.
...
SMAPI Events
    ...
    │     ├─ DepositHoursPolicy → DepositCalculator
    ...
    │
    └─ Saving ─►  CalendarHandlers.OnSavingHook            ← Pattern S (FIRST: sleep-stop + settle)
    │                └─ ShiftOrchestrator.StopForSleepAndSettle():
    │                      (a) mid-work  → stop at sleep time; mail collected/undelivered items + refund (U)
...
- `OnSavingHook(sender, args)` — subscribed to `GameLoop.Saving`; calls `ShiftOrchestrator.StopForSleepAndSettle()` ahead of persistence (Pattern S).
- Lives in `Dayswork/Orchestration/`. No Harmony patches. *(MAINT-U15-01/03)*

### RecurringContractScheduler *(Mod — M-13, promoted from stub)*
- `OnDayStarted` is the full guard chain (Pattern R): festival gate → config lock → rain-rate → current preview-hours policy/deposit → affordability gate → deduct/start. Single-active-contract invariant assumed (DEV-U15-01). *(BR-DAY-*, BR-AFF-*, BR-CTR-02)*

### ShiftOrchestrator *(Mod — M-12, extended)*
- **Removes** the direct `Saving` subscription; **adds** `StopForSleepAndSettle()` (Pattern S, branches a/b).
...
- Reuses `DepositPlanner`/multi-trip loop (M/N) and the whole worker loop unchanged for normal completion; the sleep-stop path mails collected-but-undelivered items without running a new deposit plan. *(BR-FF-*, BR-REF-05)*
...
### CapabilityEvaluator / RateCalculator / DepositHoursPolicy / DepositCalculator / RefundCalculator *(Core — reused/centralized)*

- `DepositHoursPolicy` centralizes the current flat 1.0-hour deposit preview rule so the hire summary and recurring day-start deposits stay consistent until the raw tile estimator is revisited.
...
| PBT-03 (blocking) | Compliant | Refund-formula reuse (U-05) + deposit-hours policy invariant; sleep-stop settlement is SMAPI orchestration and covered by example tests/playtest |
