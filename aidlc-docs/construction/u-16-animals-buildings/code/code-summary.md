...
- Playtest fix Step 35 corrects the sheep harvest audio cue from the invalid `Shears` id to vanilla `scissors` and adds `Dayswork.Tests/Orchestration/AnimalCollectSoundTests.cs` so future cue-name regressions are caught by the automated suite.
