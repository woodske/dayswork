...
| **Pain points** | Watering 80 crop tiles every morning eats 15+ real minutes. Harvesting on a Fall 28 strawberry day is exhausting. Late-game farms get overwhelming. |
| **Goals when using Dayswork** | (1) Hand off the boring chores. (2) See visible, satisfying work happening. (3) Never lose items. (4) Predictable, fair economics — no surprise gold drain. (5) Skip nothing on festival/rainy days. |
...
| **Context** | A generic, unnamed hired NPC. Spawns at the farm entrance at 6am on contract days. Operates with a one-shot capability snapshot of the player's tool levels (FR-TOOL-01). Invulnerable to player weapons (FR-NPC-02). Constrained to outdoor tiles + warps into buildings (FR-WORK-09). |
| **Motivations (anthropomorphized)** | Complete assigned tasks within capability. Deposit collected items safely. Don't drop anything. Get home before 8pm. |
...
| **Goals encoded as behavior** | Priority-ordered task execution (FR-WORK-03). Capability-based silent skipping (FR-SKIP-01/02). Hybrid stuck escalation (FR-WORK-12). Atomic shift completion on early player sleep (FR-DAY-02). Deposit-run completion regardless of 8pm cap (FR-WORK-06). |
...
| **Goals** | (1) Pure business logic (rate math, deposit/refund, zone-tile intersection, save-data DTOs) is isolated and unit-testable without the game. (2) All user-visible strings are routed through SMAPI's i18n so translators contribute without touching C#. |
