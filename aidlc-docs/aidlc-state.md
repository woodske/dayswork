...
- **Current Stage**: U-13B Farmer Worker + Tool Visuals — **Code Generation Part 1 (Plan) COMPLETE, AWAITING APPROVAL.** FD + NFR Requirements + NFR Design approved 2026-05-21. Plan at [u-13b-farmer-worker-tool-visuals-code-generation-plan.md](construction/plans/u-13b-farmer-worker-tool-visuals-code-generation-plan.md) — 14 steps (1 Core type + test, 2 appearance, FarmhandWorker, WorkerMovementDriver, WorkerRenderer, ToolSwapAnimator, ShiftOrchestrator seam re-point, ModEntry, build/test/docs). Deletes FarmhandNpc + PathFindControllerAdapter. Highest-risk steps 5–7 (standalone Farmer create/drive/draw) play-test-flagged. On approval → Part 2 generation.
...
- **DEV-01** [U-13B / FR-NPC-01]: Worker re-founded on `Farmer` (randomized character-creation appearance) instead of the recolored-NPC placeholder, to deliver authentic player-style tool animations (FR-WORK-10/S-07) and to make the post-V1 roadmap (energy bar, worker-owned tools, food/buffs — all Farmer-native) tractable. **Now lands in U-13B** (split out of U-13 on 2026-05-21). Trade-off accepted: U-13B rebuilds movement (custom path-follower, not PathFindController), depth-sorted drawing, and manual hit-detection for the ouch emote. Dialogue becomes additive custom work later. Pre-decided design captured in [unit-of-work.md](inception/application-design/unit-of-work.md) U-13B entry; full design formalizes in U-13B's Functional Design stage.
