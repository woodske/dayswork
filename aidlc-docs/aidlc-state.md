...
- **Current Stage**: U-13B Farmer Worker + Tool Visuals — **NFR Requirements COMPLETE, AWAITING APPROVAL.** Functional Design approved 2026-05-21 (all 6 questions = A). NFR artifacts at [construction/u-13b-farmer-worker-tool-visuals/nfr-requirements/](construction/u-13b-farmer-worker-tool-visuals/nfr-requirements/) (nfr-requirements, tech-stack-decisions). No new frameworks; only new Core type is `WorkerTool` (table-tested). Deferred to NFR Design: **TS-U13B-01** movement smoothness cadence (per-tick vs sampled vs interpolated). On approval → NFR Design for U-13B (Infrastructure Design SKIP).
