...
- **Current Stage**: U-13B Worker Actor + Task Visuals — **Code Generation REVIEW CHANGE COMPLETE, AWAITING PLAY-TEST / APPROVAL.** FD + NFR Requirements + NFR Design approved 2026-05-21. Code plan at [u-13b-farmer-worker-tool-visuals-code-generation-plan.md](construction/plans/u-13b-farmer-worker-tool-visuals-code-generation-plan.md) — original 14 steps complete plus review-change Steps 15–23. User rejected the Farmer-backed worker architecture after play-testing; implementation is now back to an NPC worker added to the normal farm character list with Stardew-Squad-style callback-free NPC task animation and world `TemporaryAnimatedSprite` tool swings for axe/pickaxe/watering-can/scythe tasks. Preserved/fixed behavior now includes nearest outdoor routing, resource-clump handling, navigation/action SMAPI diagnostics, worker-passable BFS fallback that routes around farm buildings, deterministic regular stone removal, material chunk debris collection for wood/hardwood/stone/ore drops, delayed tree-fall debris sweeps for wood spawned after the fall animation, a short visible morning entrance hold, visible final exit walk past the farm entrance after deposit, menu/time pause gating, and repeated axe swings until vanilla tree removal completes. `dotnet build`: 0 errors / 0 warnings, auto-deployed to `Mods/Dayswork`. `dotnet test`: 184 passed, 1 expected skipped. On approval → U-14 Output Pipeline construction loop.
...
- **DEV-01** [U-13B / worker actor decision]: Farmer re-founding was implemented and play-tested, then rejected due to standalone Farmer rendering/body-pose/tool-callback risk. Current direction is an NPC-backed worker in the normal farm character list, with custom callback-free task animation and explicit task execution. Post-v1 worker energy/tools/food/buffs should be modeled explicitly rather than depending on hidden `Farmer` behavior.
