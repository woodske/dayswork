...
| [multiplayer-and-ownership.md](multiplayer-and-ownership.md) | `Farmer.IsLocalPlayer` is identity-based and gates machine collect + tree XP (so the action farmer must keep the host's id); where `gainExperience` sends XP for local/remote/offline farmers; `Building.owner` is set by `buildStructure`; `Building.id` is a serialized, synced per-instance `NetGuid` (survives moving, dies with demolition) and `BuildCondition` defaults to always-available; `GetPlayer` vs `getFarmer`; `getShippingBin`/`shipItem(who)`; `FarmerTeam.GetMoney`; painting needs a `Data/PaintData` entry (no vanilla paint bucket exists); `Character.modData`/`AnimatedSprite.textureName` sync; `GameServer.kick`; SMAPI peer events and `PerScreen`. |
