...
| [dayswork-2.0-review-fixes](dayswork-2.0-review-fixes.md) — 11 review findings: lifecycle, item safety, compatibility, owner scheduling, and client state | `IN PROGRESS 2026-09-08` — **R1–R11 all implemented in code**: lifecycle isolation, shopping settlement, compatibility gating/cleanup, offline scheduling, exact tree-drop attribution, authoritative contract state on every answer, remote upgrade state on every entry path, host-quoted contract terms with a requote before any charge, the current suspension state in the handshake (landed with R1–R6, `cf96b9b`), and owner-addressed shift-start/exhaustion/unreachable notices. Focused review suite: 79 passing; full suite: 865 passing, one intentional skip. **Protocol version is 4** (R7 and R9 each added message fields). What remains is verification only: the in-game/transport acceptance rows still open on R3/R4/R6/R7/R9/R11, then the expanded S1–S32 smoke matrix | Built phases of dayswork-2.0 | `rg -n Dayswork2Review Dayswork.Tests` must discover regressions for R1–R11; `dotnet test Dayswork.Tests/Dayswork.Tests.csproj -p:EnableModDeploy=false --filter FullyQualifiedName~Dayswork2Review` must run them, not return a zero-test pass. Then run the full suite and expanded S1–S32 smoke matrix. |
