...
| [dayswork-2.0-review-fixes](dayswork-2.0-review-fixes.md) — 11 review findings: lifecycle, item safety, compatibility, owner scheduling, and client state | `NOT STARTED` — review and corrective plan written 2026-09-08; four P1 findings, seven P2 findings. R6's narrow Harmony design approved 2026-09-08, replacing the proposed per-location lease; no implementation changes | Built phases of dayswork-2.0 | `rg -n Dayswork2Review Dayswork.Tests` must discover regressions for R1–R11; `dotnet test Dayswork.Tests/Dayswork.Tests.csproj -p:EnableModDeploy=false --filter FullyQualifiedName~Dayswork2Review` must run them, not return a zero-test pass. Then run the full suite and expanded S1–S32 smoke matrix. |
